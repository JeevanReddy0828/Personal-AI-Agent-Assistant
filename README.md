...
- Web-UI panels: a Scheduled-jobs panel (view/add/remove/enable), an Agent-runs history panel, a Map panel (plot a place or `A to B` route on OpenStreetMap), and a memory-vault browser (search your Obsidian notes, open them in a rendered preview, and click through wiki-link backlinks/outlinks), alongside the control room and live metrics.
